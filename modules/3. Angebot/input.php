...
use redaxo_eingabekomponenten\{WYSIWYGEditor, Inputfield, Checkbox, Bildauswahl};
...
$farbe->setType('color');
$output .= '<div class="form-group">' . $farbe->getHTML() . '</div>';

$deck = new Inputfield("Deckkraft",  ['deckkraft'],  $sliceId, 2);
$deck->setType('range');
$deck->setSettings(array("min" => "0", "max" => "100", "value" => $deckkraft, "step" => "1"));
$deck->addStyles(array('max-width' => '250px'));
$output .= '<div class="form-group">' . $deck->getHTML() . '</div>';
...
$borderRadius->addStyles(array('max-width' => '250px'));
$output .= '<div class="form-group">' . $borderRadius->getHTML() . '</div>';

$bildauswahl = new Bildauswahl("Bild",  ['bild'],  $sliceId, 2);
$output .= '<div class="form-group">' . $bildauswahl->getHTML() . '</div>';
